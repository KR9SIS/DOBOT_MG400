...
class RobotUI(object):
    ...
    def __init__(self):
        ...
        # Connect/DisConnect
        self.button_connect = self.set_button(
            master=self.frame_robot,
            text="Connect",
            rely=0.6,
            x=630,
        ...
        # Dashboard Function
        self.frame_dashboard = LabelFrame(
            self.root,
            text="Dashboard Function",
            labelanchor="nw",
            bg="#FFFFFF",
            pady=10,
            width=870,
        ...
        # Enable/Disable
        self.button_enable = self.set_button(
            master=self.frame_dashboard,
            text="Enable",
            rely=0.1,
            x=10,
            command=self.enable,
        ...
        self.set_button(
            self.frame_dashboard, "Confirm", rely=0.55, x=350, command=self.confirm_do
        )

        # Move Function
        self.frame_move = LabelFrame(
            self.root,
            text="Move Function",
            labelanchor="nw",
            bg="#FFFFFF",
        ...
        self.set_button(
            master=self.frame_move, text="MovJ", rely=0.05, x=410, command=self.movj
        )
        self.set_button(
            master=self.frame_move, text="MovL", rely=0.05, x=500, command=self.movl
        )
        ...
        self.set_button(
            self.frame_feed, "Clear", rely=0.71, x=487, command=self.clear_error_info
        )

        # Log
        self.frame_log = LabelFrame(
            self.frame_feed_log,
            text="Log",
            labelanchor="nw",
            bg="#FFFFFF",
    ...
    def movj(self):
        self.client_move.MovJ(
            float(self.entry_dict["X:"].get()),
            float(self.entry_dict["Y:"].get()),
    ...
    def joint_movj(self):
        self.client_move.JointMovJ(
            float(self.entry_dict["J1:"].get()),
            float(self.entry_dict["J2:"].get()),
    ...
    def set_feed(self, text_list, x1, x2, x3, x4):
        self.set_button_bind(
            ...
            rely=0.2,
            x=x1,
            command=lambda: self.move_jog(text_list[0][0]),
        )
        self.set_button_bind(
            self.frame_feed,
            text_list[0][1],
            rely=0.3,
            x=x1,
            command=lambda: self.move_jog(text_list[0][1]),
        )
        self.set_button_bind(
            self.frame_feed,
            text_list[0][2],
            rely=0.4,
            x=x1,
            command=lambda: self.move_jog(text_list[0][2]),
        )
        self.set_button_bind(
            self.frame_feed,
            text_list[0][3],
            rely=0.5,
            x=x1,
            command=lambda: self.move_jog(text_list[0][3]),
        )
        ...
        self.set_button_bind(
            ...
            rely=0.2,
            x=x4,
            command=lambda: self.move_jog(text_list[2][0]),
        )
        self.set_button_bind(
            self.frame_feed,
            text_list[2][1],
    ...
    def feed_back(self):
        ...
        while True:
            ...
            if hex((a["test_value"][0])) == "0x123456789abcdef":
                ...
                self.label_di_input["text"] = bin(a["digital_input_bits"][0])[2:].rjust(
                    64, "0"
                )
                self.label_di_output["text"] = bin(a["digital_outputs"][0])[2:].rjust(
                    64, "0"
                )
